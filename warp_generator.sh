...
# ---------------------------------------------------------------------------
# AmneziaWG 2.0 параметры для Cloudflare WARP
#
# Cloudflare WARP использует стандартный WireGuard на своей стороне:
#   - H1..H4 = 1,2,3,4  (стандартные WG-идентификаторы пакетов, без диапазонов)
#   - S1..S4 = 0         (peer не добавляет префиксы, ненулевые значения ломают туннель)
#   - I1                 (CPS-пакет отправляется клиентом до handshake — работает)
#   - Jc/Jmin/Jmax       УДАЛЕНЫ в AWG 2.0
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# JSON для AmneziaVPN (контейнер amnezia-awg, AWG 2.0)
# Поле last_config — строка с содержимым .conf файла
# ---------------------------------------------------------------------------
...
AMNEZIA_JSON=$(jq -n \
    --arg last "$AWG_JSON" \
    --arg name "Cloudflare WARP (AWG 2.0)" \
    '{
        containers: [
            {
...
[ -t 1 ] && echo "########## СТРОКА ДЛЯ AMNEZIAVPN (AWG 2.0) ##########"
echo "$VPN_KEY"
[ -t 1 ] && echo "########### КОНЕЦ СТРОКИ ДЛЯ AMNEZIAVPN ###########"

echo -e "\n\n\n"
[ -t 1 ] && echo "########## НАЧАЛО КОНФИГА (AmneziaWG 2.0) ##########"
echo "${conf}"
[ -t 1 ] && echo "########### КОНЕЦ КОНФИГА ###########"

echo -e "\n"
conf_base64=$(echo -n "${conf}" | base64 -w 0)
echo "Скачать конфиг файлом: https://immalware.vercel.app/download?filename=WARP_AWG2.conf&content=${conf_base64}"
echo "Импортируйте конфиг в приложение AmneziaVPN (версия 4.8.12.9+) или AmneziaWG 2.0!"
echo -e "\n"
